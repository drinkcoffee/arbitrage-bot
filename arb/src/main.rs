...
fn cli() -> Command {
    Command::new("arb")
        .about("Arbitrage Bot & associated helpers")
        ...
        .subcommand(
            Command::new("erc20")
                .about("ERC20 contract interactions")
                .args_conflicts_with_subcommands(true)
                .flatten_help(true)
                .args(push_args())
                .subcommand(Command::new("symbol").args(push_args()))
        )
}
...
fn main() {
    ...
    match matches.subcommand() {
        ...
        Some(("erc20", sub_matches)) => {
            let stash_command = sub_matches.subcommand().unwrap_or(("push", sub_matches));
            match stash_command {
                ("symbol", sub_matches) => {
                    let message = sub_matches.get_one::<String>("message");
                    println!("Applying {message:?}");
                    erc20_symbol_command().ok();
                }
                ("pop", sub_matches) => {
                    let stash = sub_matches.get_one::<String>("STASH");
                    println!("Popping {stash:?}");
