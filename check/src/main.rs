...
async fn stuff4() -> Result<()> {
    ...
    let erc20_contract = IERC20::new(imx_addr, provider);

    let symbol_builder = erc20_contract.symbol();
    let symbol = symbol_builder.call().await?.sym;
    println!("Symbol: {symbol}");

    let total_supply_builder = erc20_contract.totalSupply();
    let total_supply = total_supply_builder.call().await?.supply.to_string();
    println!("Total Supply: {total_supply}");
